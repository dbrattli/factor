...
let rx_return_test () =
    ...
    let observable = Factor.Builder.rx { return 42 }
...
let rx_return_from_test () =
    ...
    let observable = Factor.Builder.rx { return! Rx.ofList [ 1; 2; 3 ] }
...
let return_single_value_test () =
    let tc = TestCollector<int>()
    Factor.Builder.ret 42 |> Rx.subscribe tc.Observer |> ignore
    shouldEqual [ 42 ] tc.Results
    shouldBeTrue tc.Completed
...
let builder_empty_test () =
    let tc = TestCollector<int>()
    Factor.Builder.zero () |> Rx.subscribe tc.Observer |> ignore
    shouldEqual [] tc.Results
    shouldBeTrue tc.Completed
...
let yield_from_identity_test () =
    ...
    source |> Rx.subscribe tc.Observer |> ignore
    shouldEqual [ 1; 2; 3 ] tc.Results
