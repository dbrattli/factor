...
let timer_emits_zero_after_delay_test () =
    ...
    let _disp = Reactive.timer 50 |> _.Subscribe(tc.Observer)
    sleep 200
...
let timer_completes_after_emission_test () =
    let tc = TestCollector<int>()
    let _disp = Reactive.timer 30 |> _.Subscribe(tc.Observer)
    sleep 150
    shouldBeTrue tc.Completed

let timer_disposal_prevents_emission_test () =
    let tc = TestCollector<int>()
    let disp = Reactive.timer 100 |> _.Subscribe(tc.Observer)
    sleep 20
    disp.Dispose()
    sleep 200
...
let interval_emits_incrementing_values_test () =
    let tc = TestCollector<int>()
    let disp = Reactive.interval 30 |> _.Subscribe(tc.Observer)
    sleep 130
    disp.Dispose()
    sleep 50
...
let timer_via_facade_test () =
    let tc = TestCollector<int>()
    Reactive.timer 50 |> _.Subscribe(tc.Observer) |> ignore
    sleep 150
    shouldEqual [ 0 ] tc.Results
    shouldBeTrue tc.Completed
