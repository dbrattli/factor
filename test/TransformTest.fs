...
let flat_map_monad_law_left_identity_test () =
    ...
    Rx.single 42 |> Rx.flatMap f |> Rx.subscribe tc1.Observer |> ignore

    let tc2 = TestCollector<int>()
    f 42 |> Rx.subscribe tc2.Observer |> ignore
...
let flat_map_monad_law_right_identity_test () =
    let tc1 = TestCollector<int>()
    Rx.single 42 |> Rx.subscribe tc1.Observer |> ignore

    let tc2 = TestCollector<int>()
    Rx.single 42 |> Rx.flatMap Rx.single |> Rx.subscribe tc2.Observer |> ignore
...
let flat_map_monad_law_associativity_test () =
    ...
    let tc1 = TestCollector<int>()
    m |> Rx.flatMap f |> Rx.flatMap g |> Rx.subscribe tc1.Observer |> ignore
