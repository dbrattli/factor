    ...
    { Spawn =
        fun downstream ->
            Process.spawnOp (fun () ->
                ...
                let otherSelf: Observer<'U> = { Pid = Process.selfPid (); Ref = otherRef }
                other.Spawn(otherSelf) |> ignore
                let sourceSelf: Observer<'T> = { Pid = Process.selfPid (); Ref = sourceRef }
    ...
    { Spawn =
        fun downstream ->
            ...
            Process.spawnOp (fun () ->
                ...
                let rec loop (buffer: 'T list) = actor {
                    let! msg = Process.recvMsg<'T> ref
    ...
    { Spawn =
        fun downstream ->
            Process.spawnOp (fun () ->
                ...
                let sourceRef = Process.makeRef ()
                let samplerSelf: Observer<'U> = { Pid = Process.selfPid (); Ref = samplerRef }
