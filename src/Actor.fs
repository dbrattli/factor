...
type ActorContext<'Msg> =
    { Recv: unit -> Actor<'Msg, 'Msg> }
...
type ActorBuilder() =
    ...
    member _.Return(value: 'T) : Actor<'Msg, 'T> =
        { Run = fun cont -> cont value }

    member _.ReturnFrom(actor: Actor<'Msg, 'T>) : Actor<'Msg, 'T> = actor

    member _.Zero() : Actor<'Msg, unit> =
        { Run = fun cont -> cont () }

    member _.Delay(f: unit -> Actor<'Msg, 'T>) : Actor<'Msg, 'T> =
        { Run = fun cont -> (f ()).Run cont }
